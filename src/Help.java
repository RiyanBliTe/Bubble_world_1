...
public class Help
{
	...
	public void draw(Graphics2D g){
		...
		g.fillOval(GamePanel.WIDTH / 3, GamePanel.HEIGHT / 3, 450, 200);
		g.setColor(new Color(0,0,255));
		g.setFont(new Font("Jokerman",Font.BOLD,50));
		long length = (int)g.getFontMetrics().getStringBounds("BUBBLE",g).getWidth();
		g.drawString("BUBBLE", (int)GamePanel.WIDTH / 3 + length / 2, (int)GamePanel.HEIGHT / 2 - 40 );
		g.setFont(new Font("Verdana",Font.BOLD,30));
		length = (int)g.getFontMetrics().getStringBounds("WORLD",g).getWidth();
		g.drawString("WORLD", (int)GamePanel.WIDTH / 3 + length + 30, (int)GamePanel.HEIGHT / 2 + 40);
		...
		g.fillRect(5, 390, 14, 14);
		g.setColor(Color.CYAN);
		g.fillRect(5, 414, 10, 10);
		g.setColor(Color.RED);
		g.fillRect(5, 435, 10, 10);
		g.setColor(Color.WHITE);
		g.drawString("-  +1 power", 20, 380);
		g.drawString("-  +2 power", 22, 402);
		...
		g.drawString(Who, GamePanel.WIDTH - 200, GamePanel.HEIGHT - 10);
	}
}